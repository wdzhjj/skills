...
####  EXPLAIN			
		列：
			...
			type：	
				...
				const/system
					当mysql能对查询的某部分进行优化并将其转换成一个常量时，就会使用这种访问类型
				NULL
					mysql能在优化阶段分解查询语句
		...
		备份数据
			1/ 生成逻辑备份
				...
				缺点:
					...
					逻辑备份的成本很高
			2/ 符号分割文件备份
				select info outfile 以符号分隔文件格式创建数据的逻辑备份
				备份和还原数据快
				只能备份到运行mysql服务器的机器上的文件中
				运行mysql的系统用户必须由文件目录的写权限
				不能覆盖已经存在的文件
				不能直接导出到压缩文件中
				某些情况下很难进行正确的导入导出 （非标准字符集）
			3/ 文件系统快照
